...
Enemy::Enemy(std::string id, std::string name, int maxHealth, std::uint32_t seed)
    : id(std::move(id)), name(std::move(name)), maxHealth(std::max(0, maxHealth)),
      ...
      darkCharge(0), belialBeamBonus(0), repeatedIntentCount(0),
      belialPossessed(false), belialForceUltimate(false),
      previousIntentType(EnemyIntentType::Stunned),
      archetype(archetypeFromId(this->id)), randomEngine(seed)
{
    if (archetype == EnemyArchetype::FungiBeast)
    {
        deathVulnerable = 2;
    }
    if (archetype == EnemyArchetype::Lagavulin)
    {
        metallicize = 8;
    }
    chooseInitialIntent();
...
EnemyArchetype Enemy::archetypeFromId(const std::string& enemyId) const
{
    ...
    if (enemyId == "belial") return EnemyArchetype::Belial;
...
void Enemy::chooseInitialIntent()
{
    ...
    case EnemyArchetype::Belial:
    {
        EnemyIntent next{EnemyIntentType::DefendBuff, "终极战斗仪·守", "获得 12 点护甲，并洗入 2 张黑暗侵蚀。", 0, 1, 12};
        next.darkErosion = 2;
        setIntent(next);
        break;
    }
...
void Enemy::endTurn()
{
    ...
    if (archetype == EnemyArchetype::Belial)
    {
        darkCharge = std::min(3, darkCharge + std::max(0, 1 - weak));
    }
}
...
void Enemy::chooseBelialIntent()
{
    if (belialForceUltimate || darkCharge >= 3)
    {
        belialForceUltimate = false;
        EnemyIntent next{EnemyIntentType::Attack, "帝斯修姆光线·极", "造成 6×6 点伤害，并消耗全部黑暗能量。", 6, 6};
        setIntent(next);
        return;
    }
    ...
    if (patternStep == 1)
    {
        EnemyIntent next{EnemyIntentType::DefendBuff, "黑暗护盾", "获得 4 层尖刺护甲和 18 点格挡。", 0, 1, 18, 0};
        setIntent(next);
    }
    else if (patternStep == 2)
    {
        EnemyIntent next{EnemyIntentType::Attack, "帝斯修姆光线", "造成伤害，并消耗全部黑暗能量。", 30 + belialBeamBonus};
        setIntent(next);
    }
    else
    {
        EnemyIntent next{EnemyIntentType::AttackDefend, "死亡之镰·改", "造成 10×2 点伤害并施加 2 层虚弱。", 10, 2};
        next.weak = 2;
        setIntent(next);
    }
}
...
void Enemy::triggerBelialPossession()
{
    if (belialPossessed || currentHealth * 2 > maxHealth)
    {
        return;
    }
...
int Enemy::takeDamage(int amount)
{
    ...
    if (archetype == EnemyArchetype::Belial)
    {
        triggerBelialPossession();
    }

    if (unblocked > 0 && archetype == EnemyArchetype::Lagavulin &&
    ...
    if (currentHealth > 0 && archetype == EnemyArchetype::SlimeBoss &&
...
{
    intent = attack("攻击", std::max(0, amount), "造成固定伤害。");
}
...
int Enemy::getDarkCharge() const { return darkCharge; }
void Enemy::consumeDarkCharge() { darkCharge = 0; }

void Enemy::resolveSplit()
{
    if (archetype != EnemyArchetype::SlimeBoss)
    {
        return;
    }
...
{
    return archetype == EnemyArchetype::Belial ? belialAttackDamage(intent.damage)
                                               : intent.damage;
}
...
std::string Enemy::getPowerDescription() const
{
    ...
    case EnemyArchetype::Belial:
        return "黑暗蓄能 " + std::to_string(darkCharge) + "/3";
    default:
        return {};
    }
}
