...
> 现状以 [实现现状与差距.md](实现现状与差距.md) 为准。当前 `src/app/Game` 只包含 `Menu/Map/Event/Battle/Rest/Shop/GameOver` 场景；`CardReward`、`Treasure`、`ActResult`、`MapState` 等仍是计划中的接口。
...
- D 负责当前地图节点的生成、连线和可达性；问号节点解析尚未接入。
...
当前 `Event` 节点由 `Game` 调用 `EventSystem`；问号解析、宝箱和多场景分派仍未接入。地图生成器当前接收层数而不是种子，内部随机性尚未统一到 `GameState::seed`。
